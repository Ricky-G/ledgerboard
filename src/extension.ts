...
export function activate(context: vscode.ExtensionContext): void {
  ...
  status.name = 'Kanban Ledger';
  status.text = '$(project) Kanban Ledger';
  status.tooltip = 'Open your local-first Markdown Kanban board';
  status.command = 'kanbanLedger.openBoard';
  status.show();

  context.subscriptions.push(
    status,
    vscode.commands.registerCommand('kanbanLedger.initializeBoard', (uri?: vscode.Uri) => initializeBoard(context, uri)),
    vscode.commands.registerCommand('kanbanLedger.openBoard', () => openBoard(context)),
    vscode.commands.registerCommand('kanbanLedger.addOutcome', () => addOutcome(context)),
    vscode.commands.registerCommand('kanbanLedger.validateBoard', () => validateBoard()),
    vscode.commands.registerCommand('kanbanLedger.openStandard', () => openStandard(context)),
  );
}
...
async function initializeBoard(context: vscode.ExtensionContext, uri?: vscode.Uri): Promise<void> {
  try {
    ...
    void vscode.window.showInformationMessage(`Kanban Ledger initialized in ${repository.name}. ${detail}`);
    BoardPanel.show(context, repository);
  } catch (error) {
    void vscode.window.showErrorMessage(`Kanban Ledger initialization failed: ${errorMessage(error)}`);
  }
}
...
async function openBoard(context: vscode.ExtensionContext): Promise<void> {
  try {
    ...
    void vscode.window.showErrorMessage(`Kanban Ledger could not open the board: ${errorMessage(error)}`);
  }
}
...
async function addOutcome(context: vscode.ExtensionContext): Promise<void> {
  try {
    ...
    void vscode.window.showErrorMessage(`Kanban Ledger could not add an outcome: ${errorMessage(error)}`);
  }
}
...
async function validateBoard(): Promise<void> {
  try {
    ...
    void vscode.window.showInformationMessage(
      `Kanban Ledger is valid: ${validation.cardCount} cards, ${validation.config.entities.length} entities, ${validation.historyEvents.length} history events.`,
    );
  } catch (error) {
    void vscode.window.showErrorMessage(`Kanban Ledger validation failed: ${errorMessage(error)}`);
  }
}
...
async function chooseExistingBoard(): Promise<BoardRepository | undefined> {
  ...
  if (repositories.length > 1) {
    const selected = await vscode.window.showQuickPick(
      ...
      { title: 'Open Kanban Ledger', placeHolder: 'Choose a Markdown board bundle' },
    );
  ...
  if (discovery.invalid.length > 0) {
    ...
    const action = await vscode.window.showErrorMessage(
      `Kanban Ledger found ${invalid.repository.name}, but it is invalid: ${invalid.message}`,
      'Open BOARD.md',
      'Choose folder',
    );
  ...
  const action = await vscode.window.showInformationMessage(
    'No Kanban Ledger bundle was found in this workspace.',
    'Initialize board',
    'Choose folder',
  );
  if (action === 'Initialize board') {
    await vscode.commands.executeCommand('kanbanLedger.initializeBoard');
    return undefined;
  }
  if (action === 'Choose folder') {
    return chooseBoardFolder();
...
async function chooseBoardFolder(): Promise<BoardRepository | undefined> {
  const selected = await vscode.window.showOpenDialog({
    ...
    canSelectMany: false,
    title: 'Choose a Kanban Ledger folder',
...
async function chooseTargetFolder(uri?: vscode.Uri): Promise<vscode.Uri | undefined> {
  ...
  if (folders.length > 1) {
    const selected = await vscode.window.showWorkspaceFolderPick({ placeHolder: 'Choose where to initialize Kanban Ledger' });
    return selected?.uri;
  }
  ...
  const selected = await vscode.window.showOpenDialog({
    ...
    canSelectFolders: true,
    canSelectMany: false,
    title: 'Choose a folder for Kanban Ledger',
