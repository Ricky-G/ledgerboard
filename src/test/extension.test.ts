...
suite('Extension Test Suite', function () {
	...
	suiteSetup(async () => {
		const extension = vscode.extensions.getExtension('ricky-g.kanbanledger');
		assert.ok(extension, 'Kanban Ledger extension was not discovered by the test host.');
		await extension.activate();
	});

	test('registers the public command surface', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(commands.includes('kanbanLedger.initializeBoard'));
		assert.ok(commands.includes('kanbanLedger.openBoard'));
		assert.ok(commands.includes('kanbanLedger.addOutcome'));
		assert.ok(commands.includes('kanbanLedger.validateBoard'));
		assert.ok(commands.includes('kanbanLedger.openStandard'));
	});

	test('initializes, validates, saves, and rejects stale writes', async () => {
		const root = vscode.Uri.file(path.join(os.tmpdir(), `kanban-ledger-${Date.now()}`));
	...
	test('prefers a valid workspace-root board over invalid nested bundles', async () => {
		const root = vscode.Uri.file(path.join(os.tmpdir(), `kanban-ledger-discovery-${Date.now()}`));
