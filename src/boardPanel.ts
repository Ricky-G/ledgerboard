...
export class BoardPanel implements vscode.Disposable {
  ...
  public async setRepository(repository: BoardRepository): Promise<void> {
    ...
    this.repository = repository;
    this.panel.title = `Kanban Ledger · ${repository.name}`;
    this.startWatching();
    if (this.ready) {await this.postBundle(repository, bundle);}
  }
  ...
  private async initializeHtml(): Promise<void> {
    try {
      ...
      void vscode.window.showErrorMessage(`Kanban Ledger could not open: ${errorMessage(error)}`);
      this.panel.dispose();
    }
  }
    ...
    try {
      switch (message.type) {
        ...
        case 'save':
          ...
          break;
        case 'selectBoard':
          await vscode.commands.executeCommand('kanbanLedger.openBoard');
          break;
      }
    } catch (error) {
      ...
      if (messageType === 'loadError') {
        void vscode.window.showErrorMessage(`Kanban Ledger could not load ${this.repository.name}: ${messageText}`);
      }
    }
  }
