(function startKanbanApp() {
  ...
  function handleExternalChange(fileName) {
    if (state.dirtyBoard || state.dirtyConfig || state.saveInFlight) {
      ...
      setStatus(`${fileName} changed outside Kanban Ledger. Reload to reconcile.`, "warning");
      return;
    }
    vscode.postMessage({ type: "reload" });
  }
  ...
  function applyConfig() {
    ...
    elements.boardTitle.textContent = state.config.workspace?.boardTitle || "Kanban Ledger";
