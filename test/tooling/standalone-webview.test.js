...
test('media contains one self-contained offline HTML application', () => {
  ...
  const html = fs.readFileSync(repositoryPath('media', 'index.html'), 'utf8');
  ...
  assert.equal(html, buildWebviewHtml(), 'media/index.html must match its maintainable sources.');
});
